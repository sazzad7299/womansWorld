                    ...
                    <div class="card-body">
                        ...
                        @if (isset($childCategory))
                            <form class="forms-sample" method="POST"
                                action="{{ route('admin.child-categories.update', $childCategory->id) }}"
                                ...
                                <div class="row mb-3">
                                    <div class="col-md-4">
                                        ...
                                        <select class="js-example-basic-single form-select" data-width="100%"
                                            ...
                                            @foreach ($categories as $item)
                                                <option value="{{ $item->id }}" @if ($childCategory->category_id == $item->id) {{ 'selected' }} @endif>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <select class="js-example-basic-single form-select" data-width="100%"
                                            ...
                                            @foreach ($subCategories as $item)
                                                <option value="{{ $item->id }}" @if ($childCategory->sub_category_id == $item->id) {{ 'selected' }} @endif>
                                                    {{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <input type="text" class="form-control" placeholder="{{ __('Name') }}"
                                            autocomplete="off" name="name" value="{{ $childCategory->name }}"
                                            required="" />
                                    </div>
                                </div>
                                <div class="row mb-3">
                        ...
                        @else
                            <form class="forms-sample" method="POST"
                                action="{{ route('admin.child-categories.store') }}" enctype="multipart/form-data">
                                @csrf
                                <div class="row mb-3">
                                    <div class="col-md-4">
                                        <label class="form-label">{{ __('Category') }}</label>
                            ...
                            <table id="dataTableExample" class="table">
                                <thead>
                                    <tr>
                                        ...
                                        <th width="30%">{{ __('Childcategory') }}</th>
                                ...
                                <tbody>
                                    @foreach ($childCategories as $item)
                                        <tr>
                                            ...
                                            <td>{{ $item->category }}</td>
                                            <td>
                                                <a href="{{ route('admin.child-categories.edit', $item->id) }}"
                                                ...
                                                <form action="{{ route('admin.child-categories.destroy', [$item->id]) }}"
        ...
        function getSubCategories(category_id) {
            ...
            $.ajax({
                ...
                success: function(data2) {
                    ...
                    $('#sub_category_id').find('option').remove().end().append("<option value=''>Select " +
