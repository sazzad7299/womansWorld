            ...
            <div class="col-md-12 grid-margin">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form class="forms-sample" method="POST" action="{{ route('admin.services.update',$service->id) }}"
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" placeholder="{{ __('Name') }}"
                                                autocomplete="off" name="name" value="{{ old('name',$service->name) }}" required=""
                                                id="name" />
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">{{ __('Slug') }}</label>
                                            <input type="text" class="form-control" placeholder="{{ __('Slug') }}"
                                                autocomplete="off" name="slug" value="{{ old('slug',$service->slug) }}" required=""
                                                id="slug" />
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <option value=""> Select Category</option>
                                                @foreach ($indentedCategories as $categoryId => $categoryName)
                                                    <option value="{{ $categoryId }}"  {{ $categoryId == $service->category_id ? 'selected' : '' }} >{{ html_entity_decode($categoryName) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
